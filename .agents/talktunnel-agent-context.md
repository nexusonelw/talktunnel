...
扫码连接：
1. 移动端打开 `/{uuid}`。
2. 移动端首次输入密码并验证成功后，密码保存在该浏览器的 `localStorage`，以后打开页面会自动调用 `/api/auth` 验证；验证成功之前不获取目标地址。页面底部可以清空保存的密码并退出当前连接。
3. POST `/api/auth` 校验 password 并拉取 `lanIps + port`。
4. 设备地址只保留在当前页面内存中；浏览器本地保存的是明文访问密码，密码变更导致 401 时会清除旧值。
