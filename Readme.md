...
# TalkTunnel
一个跨设备文本隧道工具，允许您在移动设备和电脑之间发送文本和文件。

## 功能特性
- **桌面应用** 显示带有服务器URL的二维码
- **移动端界面** 是一个支持PWA的网页，通过扫描二维码即可访问
- **自动文本粘贴** 在光标位置（当nut.js正常工作时）
- **回退到剪贴板** 模式，如果自动粘贴失败
- **跨平台支持**（Windows、macOS、Linux）
- **文件传输** 在设备之间
## 产品概述
这是一个能够将手机端语音输入的文字实时同步到电脑端的程序。通过手机输入法的语音识别功能，将识别的文字自动发送到电脑端的剪贴板，并粘贴到当前光标位置。
...
- 对着手机说话，语音会自动转换为文字
- 文字内容自动发送到电脑端剪贴板
- 支持长文本连续输入（百度输入法和搜狗输入法可设置长文本输入模式）

### 2. 文件传输功能
- **手机→电脑**：可在手机端选择文件或图片发送到电脑端
- **电脑→手机**：可将电脑端文件发送到手机端，文件自动保存在Download文件夹

### 3. 自动发送延迟设置
- 可根据需求调整自动发送延迟时间
- 立即发送：将延迟时间设置较低
- 批量发送：将延迟时间设置较长，积累更多内容后一次性发送
...
2. 进入语音输入模式
3. 根据输入法类型进行设置：
   - **百度/搜狗输入法**：开启长文本输入模式，保持连续语音输入
   - **微信/谷歌输入法**：建议设置延迟发送时间为3秒

### 电脑端使用

#### Mac系统
- 接收的内容会自动粘贴到当前光标位置
- 支持在各类文本编辑器中使用（如Cursor、Claude Code等）

#### Windows系统
- 接收的内容会发送到剪贴板
- **注意**：需要手动进行粘贴操作（Ctrl+V）
- 建议将自动延迟时间设置较长，以便一次性粘贴所有内容
...
## 注意事项
1. 确保手机和电脑在同一网络环境下
2. Windows用户需注意手动粘贴的操作
3. 根据实际需求调整延迟时间设置
4. 图片文件传输到电脑后会保存在剪贴板中，可直接粘贴到微信等应用

## 最佳实践
- 使用长文本输入时，选择百度或搜狗输入法
- 需要快速响应时，降低延迟时间设置
- Windows用户建议积累一定内容后再进行粘贴，避免频繁手动操作
...
如果您觉得这个项目对您有帮助，请考虑赞助支持！您的赞助将帮助项目持续维护和改进。

### 🎁 Sponsor via Afdian / 通过爱发电赞助

[![Sponsor on Afdian]()](https://afdian.com/a/talkTunnel)

[**➡️ Sponsor on Afdian (爱发电)**](https://afdian.com/a/talkTunnel)

### ☕ Buy Me a Coffee / 请我喝杯咖啡

Your support means a lot! Even a small contribution helps keep this project going.

您的支持意义重大！即使是小额赞助也能帮助项目持续发展。
